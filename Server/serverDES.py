...
def handle_client(client_socket):
    ...
    while True:
        ...
        print(f'cipher: {cipher_text}, {type(cipher_text)}')
        if not cipher_text:
            break

        plain_text = hex_to_ascii(bin2hex(encrypt(cipher_text, rkb_rev, rk_rev)))

        # Print the received message
        print(f'raw: {cipher_text}\nenc: {plain_text}')

        # Send a response back to the client
        response = input("Enter a message (8 chars): ")
        print(response)
        print(len(ascii_to_hex(response)), ascii_to_hex(response))
        plain_text = bin2hex(encrypt(ascii_to_hex(response), rkb, rk))
